...
fun HomeScreen(
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("نقش پنهان") },
                actions = {
                    IconButton(onClick = onOpenHistory) {
                        Icon(Icons.Default.History, contentDescription = "تاریخچه")
                    }
                    IconButton(onClick = onCreatePreset) {
                        Icon(Icons.Default.Add, contentDescription = "قالب جدید")
                    }
                }
            )
        }
        ...
        if (state.presets.isEmpty()) {
            Box(
                ...
                Text("هنوز قالبی نساختی. با دکمه‌ی + یکی بساز.")
            }
        } else {
            LazyColumn(
                modifier = Modifier
                ...
                items(state.presets, key = { it.id }) { preset ->
                    Card(modifier = Modifier.fillMaxWidth().padding(vertical = 6.dp)) {
                        Column(modifier = Modifier.padding(16.dp)) {
                            ...
                            Text(
                                "${preset.teams.size} تیم، ${preset.roles.size} نقش",
    ...
    presetPendingDelete?.let { preset ->
        AlertDialog(
            ...
            title = { Text("حذف قالب") },
            text = { Text("قالب «${preset.name}» حذف بشه؟") },
            confirmButton = {
                Button(onClick = {
